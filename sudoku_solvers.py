...
class Backtracker_with_IDR(Basic_Backtracker):
    ...
    def find_domain(self, puzzle, row: int, col: int) -> set:
        ...
        domain = bitarray('0' + '1' * 9)

        #check row-col
        for i in range(9):
            no1 = puzzle[row][i]
            no2 = puzzle[i][col]
            domain[no1] = domain[no2] = 0
        ...
        for i in range(box_r, box_r + 3):
            for j in range(box_c, box_c + 3):
                no = puzzle[i][j]
                domain[no] = 0
        return set(filter(domain.__getitem__, range(1, 10)))
    ...
    def get_puzzle_domains(self, puzzle: list):
        ...
        for row in range(9):
            for col in range(9):
                if puzzle[row][col] == 0:
                    var_domains[row * 9 + col] = self.find_domain(
                        puzzle, row, col)
...
class Backtracker_with_CP(Backtracker_with_IDR):
    ...
    def reduce_var_domains(self, puzzle: list, row: int, col: int,
                           var_domains: list):
        no = puzzle[row][col]
                ...
                if puzzle[i][j] == 0:
                    for no in var_domains[i * 9 + j]:
                        if self.is_consistent(puzzle, i, j, no):
                            ...
                            new_var_domains[
                                i * 9 +
                                j] = -1  # it is assigned so domain required
                            new_var_domains = self.reduce_var_domains(
                                puzzle, i, j, new_var_domains)

                            # Recurse only if all of the domains of un-assigned variables are non-empty
                            if new_var_domains == False: pass
                            else:
                                self.recursive_solver(puzzle, new_var_domains)
        ...
        if i != -1:
            for no in var_domains[i * 9 + j]:
                if self.is_consistent(puzzle, i, j, no):
                    ...
                    new_var_domains = var_domains.copy()
                    new_var_domains[
                        i * 9 + j] = -1  # it is assigned so domain required
                    new_var_domains = self.reduce_var_domains(
                        puzzle, i, j, new_var_domains)
